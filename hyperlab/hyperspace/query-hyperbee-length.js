...
//   }
  ...
  await findDefinitionWithLength(db, 35)
...
async function findDefinitionWithLength(db, length) {
  const match = []
  let i = 0
  for await ( const data of db.createReadStream() ) {
      if (data.value.length === length) match.push(data)

  }
  console.log('Resultado #', match.length)
